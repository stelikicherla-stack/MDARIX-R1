...
def create_counterfactual(investigation_id: uuid.UUID, request: CounterfactualRequest, db: Session = Depends(get_db)):
    try:
        return payload(service.execute(db, get_default_tenant_id(db), investigation_id, request))
...
def list_counterfactuals(investigation_id: uuid.UUID, db: Session = Depends(get_db)):
    return [payload(item) for item in service.list(db, get_default_tenant_id(db), investigation_id)]


@router.get("/counterfactuals/{counterfactual_id}", response_model=CounterfactualResponse)
def get_counterfactual(counterfactual_id: uuid.UUID, db: Session = Depends(get_db)):
    from backend.app.db.models.foundation import Scenario
    item = db.query(Scenario).filter(Scenario.tenant_id == get_default_tenant_id(db), Scenario.id == counterfactual_id, Scenario.assumptions["kind"].astext == "COUNTERFACTUAL").first()
    if not item:
