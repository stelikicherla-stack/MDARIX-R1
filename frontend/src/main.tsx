...
function ControlledSignature({ decision, onSigned }: { decision: any; onSigned: (result: any) => void }) {
  ...
  const sign=()=>{if(!remarks.trim()||!password){setMessage("Decision remarks and current password are required.");return} fetch(`/api/v1/governance/decisions/${decision.id}/sign`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({decision:choice,remarks,password,object_version:decision.updated_at})}).then(async r=>({ok:r.ok,data:await r.json()})).then(({ok,data})=>{setPassword("");if(!ok){setMessage(data.detail?.code==="INVALID_REAUTHENTICATION"?"Re-authentication failed. Please verify your password.":(data.detail?.message??"Signature could not be recorded."));return}setSigned(data);onSigned(data);close()}).catch(()=>{setPassword("");setMessage("Signature could not be recorded.")})};
