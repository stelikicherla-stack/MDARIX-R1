...
function AuthPage({ mode }: { mode: string }) {
  const [message,setMessage]=useState(""); const [email,setEmail]=useState(""); const [password,setPassword]=useState(""); const [name,setName]=useState(""); const [company,setCompany]=useState("");
  const submit=async(e:React.FormEvent)=>{e.preventDefault(); const endpoint=mode==="signin"?"signin":mode==="signup"?"signup":"forgot-password"; const body=mode==="signup"?{email,password,display_name:name,organization:company}: {email,password}; const response=await fetch(`/api/v1/auth/${endpoint}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)}); const data=await response.json(); setMessage(response.ok?(mode==="signin"?"Signed in. Loading MDARIX…":"Account created. Check the verification instructions."):(data.detail?.code==="PASSWORD_POLICY"?"Password must be at least 12 characters.":(data.detail?.code==="ACCOUNT_EXISTS"?"An account with this email already exists.":(data.detail?.message??"Request could not be completed.")))); if(response.ok&&mode==="signin") window.location.href="/app";};
  return <div className="auth-shell"><a className="brand" href="/">MDARIX</a><form className="auth-card" onSubmit={submit}><span className="eyebrow">{mode==="signin"?"Welcome back":"MDARIX access"}</span><h1>{mode==="signin"?"Sign in to MDARIX":mode==="signup"?"Create your MDARIX account":"Reset your password"}</h1>{mode==="signup"&&<><input aria-label="Display name" required placeholder="Full name" value={name} onChange={e=>setName(e.target.value)}/><input aria-label="Company" required placeholder="Company / organization" value={company} onChange={e=>setCompany(e.target.value)}/></>}<input aria-label="Work email" required type="email" placeholder="Work email" value={email} onChange={e=>setEmail(e.target.value)}/>{mode!=="forgot-password"&&<><input aria-label="Password" required minLength={12} type="password" placeholder="Password (minimum 12 characters)" value={password} onChange={e=>setPassword(e.target.value)}/>{mode==="signup"&&<small>Password is stored securely and never as plain text.</small>}</>}<button className="primary-action" type="submit">{mode==="signin"?"Sign in":mode==="signup"?"Create account":"Send reset instructions"}</button>{message&&<p role="status">{message}</p>}<p className="auth-links"><a href="/signup">Create account</a> · <a href="/signin">Sign in</a> · <a href="/forgot-password">Forgot password?</a></p></form></div>;
}
