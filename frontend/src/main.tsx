...
function DecisionCenterView({ investigationId, temporalMode, asOf }: { investigationId: string; temporalMode: "event" | "known"; asOf: string }) {
  ...
  const recordDecision = () => {
    if (!rationale.trim()) { setMessage("Human rationale is required before recording a decision."); return; }
  ...
  const submitReview = () => fetch(`/api/v1/investigations/decisions/${decision.id}/reviews`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ disposition: reviewDisposition, reviewer_ref: "CONTROLLED_DEVELOPMENT_REVIEWER", comments: reviewComments }) })
    .then((response) => { if (!response.ok) throw new Error(`${response.status} ${response.statusText}`); return response.json(); }).then(() => { setDecision({ ...decision, decision_status: reviewDisposition === "APPROVED" ? "DECIDED" : "REQUIRES_ADDITIONAL_EVIDENCE" }); setMessage(`Human review recorded: ${reviewDisposition}.`); }).catch((err) => setMessage(err.message));
