...
class GenAIProvider:
    def __init__(self, *, base_url=None, api_key=None, model=None, timeout_seconds=None, max_retries=None, max_input_tokens=None, max_output_tokens=None, cost_per_1k_tokens=None):
        self.base_url = base_url or os.getenv("MDARIX_GENAI_BASE_URL")
        self.api_key = api_key or os.getenv("MDARIX_GENAI_API_KEY")
        self.model = model or os.getenv("MDARIX_GENAI_MODEL", "mdarix-controlled")
    ...
    def health(self):
        return {"configured": bool(self.base_url and self.api_key), "provider": "external" if self.base_url else "controlled", "model": self.model, "configuration_version": CONFIG_VERSION, "human_review_required": True, "hidden_chain_of_thought_persisted": False}
    ...
    def complete(self, prompt: str, context: dict | None = None) -> GenAIResult:
        ...
        for attempt in range(self.retries + 1):
            try:
                ...
                return GenAIResult("external", self.model, self.model_version, output, input_tokens, output_tokens, ((input_tokens+output_tokens)/1000)*self.cost_per_1k, int((time.monotonic()-started)*1000))
